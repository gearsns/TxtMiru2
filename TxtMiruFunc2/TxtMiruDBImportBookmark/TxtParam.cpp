...
static bool ConvertToUTF16LE(LPCTSTR filename)
{
	...
	do {
		...
		if(hFile != INVALID_HANDLE_VALUE){
			...
			if(fsize > 0){
				...
				if(fsize >= 2 && data[0] == 0xff && data[1] == 0xfe){
					...
					int nLen = ::MultiByteToWideChar(CP_THREAD_ACP, 0, (char*)data, -1, nullptr,0);
					if(nLen != 0){
						HANDLE sjishHeap = ::HeapCreate(0, 0, 0);
						if (sjishHeap) {
							auto sjisdata = static_cast<LPBYTE>(::HeapAlloc(sjishHeap, HEAP_ZERO_MEMORY, sizeof(TCHAR) * (nLen + 1)));
							if (sjisdata) {
								::MultiByteToWideChar(CP_THREAD_ACP, 0, (char*)data, -1, (TCHAR*)sjisdata, nLen);
							}
		...
		if(lpData){
			::WriteFile(hFile, lpData, lstrlen((TCHAR*)lpData)*sizeof(TCHAR), &dw, nullptr);
		}
		ret = true;
	} while(0);
	if(hFile != INVALID_HANDLE_VALUE){
...
static void initTextInfoMap(CGrTxtParam::TextInfo *tim, CGrTxtParam::TextType type, LPCTSTR str = NULL)
{
						   ...
						   LPCTSTR valArcZip)
{
	...
	if(len > 0){
		qsort(&(ftMap[0]), len, sizeof(CGrTxtParam::FileTypeInfo), [](const void *p1, const void *p2){
			const CGrTxtParam::FileTypeInfo* pfti1 = (const CGrTxtParam::FileTypeInfo*)p1;
			const CGrTxtParam::FileTypeInfo* pfti2 = (const CGrTxtParam::FileTypeInfo*)p2;
...
{
	::initValueTypeMap(m_valueTypeMap, HangingCharacters      , _T(",.¤¡ABCD"                                                                     ));
	::initValueTypeMap(m_valueTypeMap, LineStartNGCharacters  , _T(",.¤¡ABCD£vx]}):;?!Þß¥~EFGHIJK]fhjlnprtz¬­®‚á‚ã‚åƒƒƒ…ƒ‡‚Áƒb"));
	::initValueTypeMap(m_valueTypeMap, LineEndNGCharacters    , _T("uwiokmy(["                                                                 ));
	::initValueTypeMap(m_valueTypeMap, LineStartSkipCharacters, _T(" @"                                                                              ));
	::initValueTypeMap(m_valueTypeMap, SeparateNGCharacters   , _T("cd"                                                                             ));
	::initValueTypeMap(m_valueTypeMap, RotateCharacters       , _T("FGefgh|€‚ƒ„…†"                                                       ));
	::initValueTypeMap(m_valueTypeMap, RRotateCharacters      , _T(""                                                                                 ));
	//
	...
	::initPointMap(m_pointsMap, BookMarkAutoSave, 1,  1     );
	...
	::initPointMap(m_pointsMap, UsePreParser    , 1,  1     );
	::initPointMap(m_pointsMap, ArcMaxFileSize  , 1,  10    );
	::initPointMap(m_pointsMap, LupePos         , 5,  0,-1,-1,-1,-1);
	::initPointMap(m_pointsMap, FileAutoReload  , 1,  0     );
	::initPointMap(m_pointsMap, UseOverlapChar  , 1,  1     );
	::initPointMap(m_pointsMap, PictPaddingNone , 1,  0     );
	::initPointMap(m_pointsMap, SpSelectionMode , 1,  1     );
	...
	::initTextInfoMap(m_textInfoMap, BrowserAppName    , _T("IEXPLORE,Firefox,Sleipnir,Sleipnir2,Mozilla,Opera,Flock,NETSCAPE"));
...
{
	Value *it = m_valueTypeMap;
	for(int len=VT_MaxNum; len>0; --len, ++it){ vs(*it); }
}
void CGrTxtParam::ForEachCharInfo(ValueSetter &vs)
{
	CharInfo *it = m_charInfoMap;
	for(int len=CT_MaxNum; len>0; --len, ++it){ vs(*it); }
}
void CGrTxtParam::ForEachTextInfo(ValueSetter &vs)
{
	TextInfo *it = m_textInfoMap;
	for(int len=TT_MaxNum; len>0; --len, ++it){ vs(*it); }
}
void CGrTxtParam::ForEachPoints(ValueSetter &vs)
{
	Points *it = m_pointsMap;
	for(int len=PT_MaxNum; len>0; --len, ++it){ vs(*it); }
}
void CGrTxtParam::ForEachCharOffset(ValueSetter &vs)
{
	auto it=m_charOffsetMap.begin(), e=m_charOffsetMap.end();
	for(;it != e; ++it){ vs(*it); }
}
...
{
	HANDLE hFile = ::CreateFile(lpFileName, GENERIC_WRITE, FILE_SHARE_WRITE, 0, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0);
	if(hFile == INVALID_HANDLE_VALUE){
		return false;
	}
	BYTE header[] = {0xff, 0xfe};
...
struct LoadValue : public CGrTxtParam::ValueSetter {
	...
	virtual void operator()(CGrTxtParam::CharInfo &info){
		...
		if(::TxtGetPrivateProfileString(_T("font"), info.key, str, sizeof(str)/sizeof(TCHAR), lpFileName)){
			if(lstrlen(str) < sizeof(info.fontName)/sizeof(TCHAR)){
	...
	if(csv.Open(offset_file)){
		...
		for(int row=0; row<max_row; ++row){
			...
			if(col_num == 3){
				...
				POINT &pos = m_charOffsetMap[csv.GetText(row, 0)];
				pos.x = array[0];
				pos.y = array[1];
			} else if(col_num == 2){
...
{
	const StringArray &string_array = m_valueTypeMap[type].array;
	int len=string_array.size();
	if(len <= 0){ return -1; }
	const std::tstring *it = &string_array[0];
...
void CGrTxtParam::SetText(TextType type, LPCTSTR lpStr)
{
	...
	std::tstring &str = m_textInfoMap[type].str;
	if(!str.empty()){
		std::replace(str, _T("\r\n"), _T(" "));
		std::replace(str, _T("\r"), _T(" "));
		std::replace(str, _T("\n"), _T(" "));
	}
...
void CGrTxtParam::GetTextList(ValueType type, std::tstring &out)
{
...
CGrTxtParam::FileType CGrTxtParam::GetFileType(LPCTSTR lpExt) const
{
	...
	if(lpExt && len > 0){
		FileTypeInfo *pfti = (FileTypeInfo*)bsearch(
			lpExt, m_fileTypeMap.data(), len, sizeof(FileTypeInfo),
			[](const void *key, const void *pdata){
				LPCTSTR pext  = (LPCTSTR)key;
				CGrTxtParam::FileTypeInfo *pfti = (CGrTxtParam::FileTypeInfo*)pdata;
