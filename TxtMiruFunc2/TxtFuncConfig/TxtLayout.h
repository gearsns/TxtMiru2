...
public:
	...
	//
	virtual int ConfigurationDlg(HWND hWnd, LPCTSTR type, LPCTSTR name, LPCTSTR lpFileName) = 0;
	//
...
public:
	...
	virtual int ConfigurationDlg(HWND hWnd, LPCTSTR type, LPCTSTR name, LPCTSTR lpFileName);
	virtual void SetInitialize();
	virtual bool IsSupported(LPCTSTR lpFileName) const;
	virtual bool IsSupported(CGrCSVText &csv) const;
	void SetFileName(LPCTSTR lpFileName);
	void SetOpenLayoutType(LPCTSTR lpType);
};

class CGrBunkoTxtLayout : public CGrCustomTxtLayout
{
public:
	CGrBunkoTxtLayout();
	virtual ~CGrBunkoTxtLayout();
	virtual LPCTSTR LayoutType() const { return Name(); }
	virtual void SetInitialize();
	static LPCTSTR Name();
};

class CGrShinShoTate2TxtLayout : public CGrCustomTxtLayout
{
public:
	CGrShinShoTate2TxtLayout();
	virtual ~CGrShinShoTate2TxtLayout();
	virtual LPCTSTR LayoutType() const { return Name(); }
	virtual void SetInitialize();
	static LPCTSTR Name();
};
